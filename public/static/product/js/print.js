...
function preview(parsedOrders,totalCopy=1,goods,goodsTwoCate,userName,businessName,type=1) {
    ...
    // }
    init(parsedOrders,totalCopy,goods,goodsTwoCate,userName,businessName,type);
    LODOP.PREVIEW();
}
//type=1 默认打印产品信息 type=2 打印物流信息
function print(parsedOrders,totalCopy=1,goods,goodsTwoCate,userName,businessName,type=1) {
...
function init(order,totalCopy,goods,goodsTwoCate,businessName,userName,type) {
    LODOP = getLodop();
    ...
    if (DEFAULT_PRINT_MODE === PRINT_MODE_SINGLE_LABEL_PER_PAGE) {
        generateOrderPrint(order, totalCopy,goods,goodsTwoCate,businessName,userName,type);
    } else if (DEFAULT_PRINT_MODE === PRINT_MODE_THREE_LABEL_PER_PAGE) {
        generateOrderPrint2(order, totalCopy,goods,goodsTwoCate,businessName,userName,type);
    }
}

//single label per page
function generateOrderPrint(order, copy,goods,goodsTwoCate,businessName,userName,type) {
