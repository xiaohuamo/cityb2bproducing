 ...
 * @mixin \think\Model
 */
class DispatchingItemBehaviorLog extends Model
{
    use modelTrait;
    ...
    public function addProducingBehaviorLog($user_id,$business_userId,$behavior_type,$delivery_date,$data)
    {
        $data = [
            ...
            'boxesNumber' =>  $data['boxesNumber'] ?? 0,
            'boxesNumberSortId' =>  $data['boxesNumberSortId'] ?? 0,
    ...
    public function getLogData($businessId,$user_id,$param)
    {
        ...
        $data = Db::name('dispatching_item_behavior_log')
            ->alias('pibl')
            ->field('pibl.*,u.name,u.nickname')
            ->leftJoin('user u','u.id = pibl.userId')
            ->where($where)
