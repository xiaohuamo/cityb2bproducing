...
class UploadFileService
{
    ...
    public function uploadLocaFile($fileName)
    {
        ...
        if (!$file) return 'Upload file does not exist!';
        ...
        if (!$fileName) return 'Image upload failed!';
        $filePath = Filesystem::disk('public')->path($fileName);
        $fileInfo = new File($filePath);
        $url = Filesystem::getDiskConfig('public', 'url') . '/' . str_replace('\\', '/', $fileName);
        $this->uploadPath = '';
    ...
    protected function setUploadInfo(string $url, int $imageType, string $name = '', string $thumbPath = '', array $headerArray = [])
    {
        ...
        $info['all_path'] = PICTURE_URL.$info['dir'];
